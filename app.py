...
# Função para centralizar métricas com parâmetros ajustáveis
def centralizar_metricas(titulo, valor, font_size=16, font_weight='normal', border_size='1px', border_radius='5px', padding='10px', border_color='#e6e6e6'):
    st.markdown(
        f"""
        <div style="
            text-align: center; 
...
with col6:
    ...
    with st.container():
        centralizar_metricas("Total de Amostras Coletadas", total_amostras, font_size=16, font_weight='normal', border_size='1px', border_radius='5px', padding='10px', border_color='#e6e6e6')

# Quantas detecções
with col7:
    total_deteccoes = dados_detec['Detecção'].sum()
    with st.container():
        centralizar_metricas("Total de Detecções", total_deteccoes, font_size=16, font_weight='normal', border_size='1px', border_radius='5px', padding='10px', border_color='#e6e6e6')

# Quantos municípios com detecção
with col8:
    municipios_com_detec = dados_detec['Municipio'].nunique()
    with st.container():
        centralizar_metricas("Municípios com Detecção", municipios_com_detec, font_size=16, font_weight='normal', border_size='1px', border_radius='5px', padding='10px', border_color='#e6e6e6')

# Quantos municípios houve coleta
with col9:
    municipios_com_coleta = dados['Municipio'].nunique()
    with st.container():
        centralizar_metricas("Municípios com Coleta", municipios_com_coleta, font_size=16, font_weight='normal', border_size='1px', border_radius='5px', padding='10px', border_color='#e6e6e6')
